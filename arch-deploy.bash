...
deploy-users ()
{
    ...
    if [[ -n "$USER" ]]; then
        ...
        if [[ -n "$USER_PASSWORD" ]]; then
            ...
            arch-chroot /mnt /bin/bash -c "echo $USER:$USER_PASSWORD | chpasswd"
        else
...
deploy-unmount ()
{
    ...
    umount -R /mnt || log "Failed to umount /mnt" err
    if [[ "$ENABLE_FULL_DRIVE_ENCRYPTION" == "yes" ]]; then
        log "Closing the encrypted partition"
        cryptsetup close root || log "Failed to close the encrypted partition" err
    fi
}
